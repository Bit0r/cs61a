...
class VendingMachine:
    ...
    def vend(self):
        if self.stock == 0:
            ...
            return f'Here is your {self.product}' + (f' and ${change} change.'
                                                     if change else '.')
...
def path_yielder(t: Tree, value):
    ...
    def path_to_value(t: Tree):
        ...
        for b in t.branches:
            for p in path_to_value(b):
                yield [label] + p

    return path_to_value(t)
...
class Mint:
    ...
    def create(self, kind: Type[Coin]):
...
def is_bst(t):
    ...
    def traverse(t: Tree):
        ...
        if not flag or t.is_leaf():
            return
        elif len(branches) > 2:
            flag = False
            return
        elif len(branches) == 2 and (bst_max(branches[0]) > t.label
                                     or bst_min(branches[1]) <= t.label):
            flag = False
            return
