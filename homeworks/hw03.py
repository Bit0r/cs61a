...
def balanced(m):
    ...
    is_balanced = total_weight(left_end) * length(left_arm) == total_weight(
        right_end) * length(right_arm)
...
def replace_loki_at_leaf(t, lokis_replacement):
    ...
    def replace(t):
        label_t = label(t)
        if is_leaf(t):
            return tree(lokis_replacement if label_t == 'loki' else label_t)

        return tree(label_t, [replace(branche_t) for branche_t in branches(t)])
...
def multiple_references_explanation():
    return """According to the inequality algorithm.
    multiplication and division will increase the error,
    while addition and subtraction will not."""
