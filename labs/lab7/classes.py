...
class TutorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent: Player):
        """
        ...
        print(f'{opponent.name} discarded and re-drew 3 cards!')
...
class TACard(Card):
    ...
    def effect(self, opponent_card: Card, player, opponent):
        """
...
class ProfessorCard(Card):
    ...
    def effect(self, opponent_card: Card, player: Player, opponent: Player):
        ...
        opponent.deck.cards = list(
            filter(
                lambda card: (opponent_card.attack, opponent_card.defense) !=
                (card.attack, card.defense), opponent.deck.cards))
        discarded = orig_opponent_deck_length - len(opponent.deck.cards)
        if discarded:
            #Uncomment the line below when you've finished implementing this method!
            print(
                f"{discarded} cards were discarded from {opponent.name}'s deck!"
            )
            return
