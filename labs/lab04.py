...
def sprout_leaves(t, leaves):
    ...
    def new_tree(t):
        root = label(t)
        if is_leaf(t):
            return tree(root, [tree(leaf) for leaf in leaves])
        else:
            return tree(root, [new_tree(sub_t) for sub_t in branches(t)])
...
def coords(fn: Callable, seq: Iterable, lower, upper):
    """
    >>> seq = [-4, -2, 0, 1, 3]
    >>> fn = lambda x: x**2
    >>> coords(fn, seq, 1, 9)
...
def add_trees(t1, t2):
    ...
    if is_leaf(t1):
        return tree(root, branches(t2))
    elif is_leaf(t2):
        return tree(root, branches(t1))
    else:
        return tree(root, [
            add_trees(*pair) for pair in zip_longest(
